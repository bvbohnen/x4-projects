...
appears to return a time in seconds (probably since epoch or similar).

TODO: maybe regular clock support, but low priority since users can
...
Commands:
  - getRealTime (<id>)
    ...
    - Likely this is time since the epoch, as defined by local OS.
...
function loc.Update_Timer(id)
    ...
    if loc.timers[id].running then
        loc.timers[id].total = now - loc.timers[id].last_check
    end
    loc.timers[id].last_check = now
end

function loc.Start_Timer(id)
    loc.Make_Timer(id)
...
function loc.Set_Alarm(id_time)
